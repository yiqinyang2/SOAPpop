...
int snpindex(SNP **snpmarkers, int numsnps, char *snpid) 
{
     ...
     // set up hash table (key=SNP name, value=index in snpmarkers)
     if (snptab==NO) {  
       snptab = YES ;
...
/*! \fn void wbuff(unsigned char *buff, int num, int g)    
    \brief Write one genotype to buffer of packed genotype data
    \param buff   Buffer
    \param num    Number of this (2-bit) datum in the buffer
    \param g      Value to store in buffer

    Each datum occupies two bits in the buffer.
...
void wbuff(unsigned char *buff, int num, int g)   {
  ...
  msk = 3 << 6 ;                             // left-justify two ones
  mm = g << 6 ;                              // left-justify this (2-bit) datum
  ones = 0XFF  ;                             // make a byte full of ones

  wnum = num/4 ;                             // byte this datum goes in
  wplace = num%4 ;                           // position of this datum in byte

  mm = mm >> (wplace * 2) ;                  // datum shifted to final position in byte
  msk = (msk >> (wplace *2)) ^ ones ;        // ones in all positions this datum does not use

  buff[wnum] &= msk ;                        // retain bits this datum does not use
  buff[wnum] |= mm ;                         // set this datum's bits
