...
//  t = strcmp(cupt -> ID, "rs10914979") ;
 ...
 for (i=0; i<numsnps; ++i) {  
  ...
  if (cupt -> weight <= 0.0)  {
    cupt -> ignore = YES ;
    logdeletedsnp(cupt->ID,"weight",deletesnpoutname);
  }
 }
...
// Adds the given block of size ncols, nrows, to the block of the given matrix X, starting at target_col, target_row.
// The given block must be ncols * nrows in size.
void
addblocktomatrix(double *X, const int target_col, const int target_row, const int x_cols, const double* block, const int block_cols, const int block_rows) {
  const double *block_idx;
  double *x_idx;
  int row = target_row;

  for( block_idx = block; block_idx < block + block_rows*block_cols; block_idx++ ) {
    if( (block_idx - block) % block_cols == 0 )
      x_idx = X + x_cols*row++ + target_col;
    *x_idx++ += *block_idx;
  }
}

// Computes the outer product of two regions of a vector and puts it into the the given matrix.
// We expect that X is a square matrix with sides of size nelts, and v is a vector with length nelts.
// v{t}_start, v{t}_stop are 0-based.
void
addouterblock(double *X, const double *v, const int nelts, const int v_start, const int v_stop, const int vt_start, const int vt_stop) {
  const double *v_idx, *vt_idx;
  double *x_idx = X;
  for( v_idx = v+v_start; v_idx <= v+v_stop; v_idx++ )
    for( vt_idx = v + vt_start; vt_idx <= v+vt_stop; vt_idx++ )
      *x_idx++ += (*v_idx) * (*vt_idx);
}
