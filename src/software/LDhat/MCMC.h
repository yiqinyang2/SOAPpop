...
//		set_moves(1, 0, 0, 1, 0, 1, 1);
		...
		for (run=1;run<=n_update;run++) 
		{
			...
			{	// Completed burn-in. Output burn-in details to screen.
				...
				printf("\nFinal Burn In LK %.3lf: ChangePoints %4i: Hotspots %4i: MapLen %.3lf",lk[0],numchangepoints,numhotspots, rmap[mydata->lseq]);

				printf("\n\nBurn In Acceptance rates:\n");
				printf("Change Block Rate =   \t %.4lf\t Move Changepoint =\t %.4lf\n", (double) nacc[0][1]/nacc[0][0], (double) nacc[1][1]/nacc[1][0]);
				printf("Split Block =         \t %.4lf\t Merge blocks =    \t %.4lf\n", (double) nacc[2][1]/nacc[2][0], (double) nacc[3][1]/nacc[3][0]);
				printf("Change Hotspot Heat = \t %.4lf\t Move Hotspot =    \t %.4lf\n", (double) nacc[4][1]/nacc[4][0], (double) nacc[5][1]/nacc[5][0]);
				printf("Insert Hotspot =      \t %.4lf\t Delete Hotspot =  \t %.4lf\n", (double) nacc[6][1]/nacc[6][0], (double) nacc[7][1]/nacc[7][0]);
				printf("Change Hotspot Scale =\t %.4lf\n",(double) nacc[8][1]/nacc[8][0]);
			...
			{	// Output a sample
				lk[0] = myLikelihood->update_lk0(mydata);
				if (!nonverbose) printf("\nRun %8i: LK %.3lf: ChangePoints %4i: Hotspots %4i: MapLen %.3lf",run,lk[0],numchangepoints,numhotspots, rmap[mydata->lseq]);
	...
	void output_acceptance_rates()
	{
		...
		fprintf(output, "Change Block Rate =   \t %.4lf\nMove Changepoint =\t %.4lf\n", (double) nacc[0][1]/nacc[0][0], (double) nacc[1][1]/nacc[1][0]);
		fprintf(output, "Split Block =         \t %.4lf\nMerge blocks =    \t %.4lf\n", (double) nacc[2][1]/nacc[2][0], (double) nacc[3][1]/nacc[3][0]);
		fprintf(output, "Change Hotspot Heat = \t %.4lf\nMove Hotspot =    \t %.4lf\n", (double) nacc[4][1]/nacc[4][0], (double) nacc[5][1]/nacc[5][0]);
		fprintf(output, "Insert Hotspot =      \t %.4lf\nDelete Hotspot =  \t %.4lf\n", (double) nacc[6][1]/nacc[6][0], (double) nacc[7][1]/nacc[7][0]);
		fprintf(output, "Change Hotspot Scale =\t %.4lf\n",(double) nacc[8][1]/nacc[8][0]);
	...
	void print_rates(data *mydata, FILE *outfile)
	{
		int i;
		double rate;
		fprintf(outfile, "%.6lf ", rmap[mydata->lseq]);
		for(i=1; i<mydata->lseq; i++)
		{
			rate = (rmap[i] - rmap[i-1]) / (mydata->locs[i+1] - mydata->locs[i]);
			fprintf(outfile, "%.6lf ", rate);
		}
		fprintf(outfile, "\n");
	}
