...
/*		check_lin(list, &k, con);
	...
	/*Count lineages which can contribute to rec events*/
		...
		avR = (double) rr/k;
		if (DEBUG) printf("\nTotal recombination length = %.0lf , Re = %.3lf", rr, avR);
	...
	/*Count pairs of lineages that can coalesce under the SMC - only needed if printing*/
		if (EVENT_PRINT) {
			...
			fprintf(ofp,"%i\t%i\t%.3lf\t%.3lf\t",k, k_eff, (double) avR/2, -log((double) k_eff+avR/2));
		}

	/*Choose time for next event*/
		told=t;
		choose_time(&t, k, avR, con);
		if (DEBUG>2) printf("\nNext event at time %.3lf\n", t);
...
FILE *ofp;
{
	...
	extern int tree_size;
			...
			j=i; while(j==i) j = 1 +((*k)+1)*ran2(); /*Choose second lineage to coalesce*/
			if (DEBUG) printf("Lineages %i and %i coalesce", i,j);

			if (SMC) {
				for (pos=1,fl=0;pos<=con->len;pos++) if (list[i]->asite[pos] && list[j]->asite[pos]) {fl=1;break;}
				if (!fl) {
					(*k)++;
					if (EVENT_PRINT) fprintf(ofp,"0\n");
					return list;
				}
			}
			...
			for (pos=1;pos<=con->len;pos++) {
				if (list[i]->asite[pos] && list[j]->asite[pos]) {/*Coalescent event at site*/
					segl[pos][1]--;
					...
					if (segl[pos][1]>1) list[j]->asite[pos]=tree_ptr[pos][node_pos]; 
					else {
						list[j]->asite[pos]=NULL;
						//printf("\rSite %5i MRCA : tally = %i",pos, ++tally);
						}
				}
				else list[j]->asite[pos] = (struct node_tree *) (list[i]->asite[pos]?list[i]->asite[pos]:list[j]->asite[pos]); /*Just copy existing*/
			}
...
FILE *ofp;
{
	...
	for (cump=0,j=j0;cump<ri;j++) cump=con->rmap[j]-con->rmap[j0]; /*Choose last point on LHS to come from parent 1*/
	j0=j-2;

	if (DEBUG > 1) printf("%i\n", j0);
	for (j=1; j<=j0; j++) {list[(*k)]->asite[j]=NULL;}/*Copy LHS to parent*/

	if (EVENT_PRINT) fprintf(ofp,"%.3lf\n",(double) log((con->rmap[j0+1]-con->rmap[j0])/2));
	...
	for (;j<=upper; j++) {list[(*k)]->asite[j]=list[i]->asite[j]; list[i]->asite[j]=NULL;}
	for (;j<=con->len;j++) {list[(*k)]->asite[j]=NULL;}
...
int len, k;
{
	...
	printf("\n\nLineages active (%i)\n\n", k);
	for (i=1; i<=k; i++) {
		printf("lineage %3i: time %.3lf: rlen %6.2lf :ANC ",\
...
int len;
{
	...
	for (n=1; n<=tree_size; n++) {
		printf("node %3i: time %.3lf ", tree[n]->node_num, tree[n]->time);
...
struct control *con;
{
	...
	nodel->rlen=0.0; nodel->nanc=1;
	for (i=1; i<=con->len; i++) if (nodel->asite[i]) break;
	if (i>con->len) {nodel->rlen = 0; nodel->nanc=0; return;} /*If no longer any ancestral material*/
	nodel->rlen -= (double) con->rmap[i];
	for (i=con->len; i>0; i--) if (nodel->asite[i]) break;
	nodel->rlen += (double) con->rmap[i];
}
...
/*	printf("\n\nTree summary");*/ 
	...
	if (TREEPRINT) {
		...
		for (i=1;i<=tree_size;i++) {
			fprintf(ofp,"\n%i\t%.3lf\t1\t%i",tree[site][i]->node_num, tree[site][i]->time, con->len);
...
/*	Read user-defined mutation model*/
	...
	if (con->mut > 0) {
	  ...
	  for (i=1; i<=4;i++) {
	    printf("\n%4c:", bases[i]);
	    for (j=1;j<=4;j++) {printf(" %.3lf", mutmat[i][j]); mutmat[i][j]/=mm[i];}
	  }
	  fclose(ifp);
	}	
...
/*Summary statistics for polymorphism*/
	for (site=1, sn=0, mhm=0; site<=con->len; site++) {
		for (i=(int) (con->asc==2 ? NPANEL+3 : 2), fi=0; i<=con->nsamp; i++) 
			if (seqs[i][site]!=seqs[(int) (con->asc==2 ? NPANEL+2 : 1)][site]) fi++;
		if (fi && (fi!=con->nsamp)) {
			sn++; 
			if ((con->theta[site])>(con->theta[1])) mhm++;
			if (fl) res->fdist[fi]++;	
		}
	...
	if (fl) {
	  ...
	  if (DEBUG) printf("\nWeighting factor for sample = %.5lf\n", exp(cf));
	}
...
struct control *con;
{
	...
	*/ 
	...
	fprintf(loc,"%i %.3lf L", ns, (double) con->rescale);
	for (i=1;i<=con->len;i++) 
		if (seg[i]) 
			fprintf(loc,"\n%.3lf", ((double)i)/((double)con->len)*con->rescale);
	fclose(loc);
...
struct control con;
{
	...
	for (i=2, an=1; i<con.nsamp; i++) an += (double) 1/i;
	for (i=1, em=0.0, es=0.0;i<=con.len; i++) {
		em+= (double) con.theta[i];
		es += (double) 1-exp(-con.theta[i]*an);
	}
	for (i=1;i<=con.nsamp;i++) res.fdist[i]/=res.cf;

	printf("\n\nResults of %i simulations\n\n", con.nrun);
	printf("Average no. mutations  = %.3lf\n", (double) res.nm/res.cf);
	printf("Expected               = %.3lf\n", (double) em*an);
	printf("Average Sn             = %.3lf\n", (double) res.sn/res.cf);
	printf("Expected               = %.3lf\n", (double) es);
	printf("Average PWD            = %.3lf\n", (double) res.pwd/res.cf);
	printf("Expected               = %.3lf\n", (double) em);
	printf("Average no. rec events = %.3lf\n", (double) res.nr/res.cf);
	printf("Expected               = %.3lf\n", (double) con.R*an);
	if (con.hyp) printf("Proportion muts HM     = %.3lf\n", (double) res.mhm/res.cf);
	printf("ESS                    = %.3lf\n", (double) (res.cf)*(res.cf)/(res.cf2));
	printf("\n\n");

	printf("Frequency distribution\n\n");
	for (i=1;i<con.nsamp-((int) con.asc==2?NPANEL+1:0);i++) printf("%3i %8.3lf\n",i,res.fdist[i]);
	printf("\n\n");
}
...
char *argv[];
{
	...
	printf("\nWatterson's const for n=%i = %.3lf\n", con->nsamp, con->w0);

	if (con->seed == 0) con->seed = -setseed();
	idum = &(con->seed);
	...
	if ((con->a)||(con->hyp)) {
	  printf("\n\nThetas for sites\n\n");
	  for (i=1;i<=con->len;i++) printf("%4i : %.5lf\n", i, con->theta[i]);
	}
	...
	if (con->rm) {
		...
		if (DEBUG) {
			printf("\n\nRecombination map:\n\n");
			for (i=1;i<=con->len;i++) printf(" %4i %5.2lf\n", i, con->rmap[i]);
		}
	}
	
	if (!(con->rm)){
		for (i=1;i<=con->len;i++) con->rmap[i]=(double) (i-1)*(con->R)/((con->len)-1);
	}


	printf("\n\nParameters: sample %i: Length %i: theta %.4lf: R %.4lf: Nrun %i : seed = %li\n", \
		con->nsamp, con->len, con->theta[1], con->R, con->nrun, con->seed);
	if (con->hyp) printf("HM: phm = %.3lf rhm = %.3lf\n", con->phm, con->rhm);
...
char *argv[];
{
	...
	for(i = 0; i < argc; i++)
	{
		...
		{ 
			in_str = argv[i];
			if(strcmp(in_str, "-s") == 0)
			{	/*User defined seed*/
				if ((i+1)<argc) con->seed = atol(argv[i+1]);
				else {printf("\nIncorrect flag use: -s seed\n"); exit(1);}
				if (con->seed > 0)
					con->seed = -con->seed;
				if (con->seed != 0) printf("\nUser-defined seed: %li\n", con->seed);
				else {printf("\nSeed > 0!\n"); exit(1);}	
			}
			...
			{	/*Include hypermutable sites: phm=Proportion, rhm=Relative rate */
				...
				printf("\nIncluding hypermutable sites: pHM = %.4lf, rHM = %.1lf\n",con->phm, con->rhm);
			}
			...
			{	/*Gene conversion model - input ratio of gene conversion to recombination and average tract length*/
				...
				printf("\n\nGene conversion model: ratio = %.3lf: geometric distribution of tract lengths with mean = %.3lf\n\n",con->cratio, con->clen);
			}
...
struct control *con;
{
	...
	for (i=1;i<=*k;i++) if (list[i]->rlen<0) {
		printf("\n\nError in rlen at %i (%lf)!!",i, (double) list[i]->rlen);
	}
	...
	for(i = 0; i < argc; i++){
		if(*argv[i] == '-'){ 
			...
			if((strcmp(in_str, "-h") == 0) || (strcmp(in_str, "-?") == 0) || (strcmp(in_str, "-help") == 0)) {
				...
				printf("                     frequency (specified: e.g. -f 2)\n");
